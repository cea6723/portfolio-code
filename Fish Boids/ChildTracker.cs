...
public class ChildTracker : MonoBehaviour
{
    ...
    /// <summary>
    /// Get the average velocity of children within a certain distance of a point
    /// - Clare Archbold
    /// </summary>
    public Vector3 GetAverageChildVelocityNear(Vector3 position, float distance = float.MaxValue)
    {
        Vector3 velocity = Vector3.zero;
        int poCount = 0;
        foreach (PhysicsObject po in GetComponentsInChildren<PhysicsObject>())
        {
            if (Vector3.Distance(po.transform.position, position) < distance && 
                Vector3.Distance(po.transform.position, position) != 0)
            {
