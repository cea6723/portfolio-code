...
public abstract class Agent : PhysicsObject
{
    ...
    /// </summary>
    protected Vector3 Flee(Vector3 targetPos, float fleeRadius)
    {
        if (Vector3.Distance(transform.position, targetPos) < fleeRadius)
        {
    ...
    /// </summary>
    protected Vector3 Flee(GameObject target, float fleeRadius)
    {
        return Flee(target.transform.position, fleeRadius);
    }

    protected Vector3 Seperate(IEnumerable<Vector3> agentPositions, Vector3 pos)
    {
        Vector3 desiredVelocity = Vector3.zero;
        float distance;

        // calculate steering force away from each neighbor
        foreach (Vector3 agentPosition in agentPositions)
        {
            if (agentPosition != pos)
            {
                distance = (agentPosition - pos).magnitude;
                desiredVelocity += (pos - agentPosition)/distance;
            }
        }

        return desiredVelocity.normalized * maxSpeed - Velocity;
        
    }

    protected Vector3 Cohere(Vector3 averagePosition)
    {
    ...
    {
        return averageVelocity.normalized * maxSpeed - Velocity;
    }
